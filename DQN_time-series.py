...
class ClassificationEnv(gym.Env):
    ...
    def step(self, action):
        true = self.labels[self.idx]
        reward = 1.0 if action == true else -1.0
        # one-step episode
        return self.features[self.idx], reward, False, True, {}
...
def train_and_evaluate():
    PARQUET      = "/Users/vardanvij/Downloads/creation/train_data_timeseries.parquet"
    EPISODES     = 10000
    BATCH_SIZE   = 64
    MEMORY_SIZE  = 10000
    LR           = 1e-3
    ...
    plt.xticks([0,1], ['Legit','Scam']); plt.yticks([0,1], ['Legit','Scam'])
    for i in [0,1]:
        for j in [0,1]:
